...
def handleRequest(Request):
	...
	newFileNames = FilesManagment.saveFiles(files, extensions, path.join(Request.path, "views", "articlesAttachments"))
	conn = connect("models/db.db")
	cursor = conn.cursor()
	cursor.execute('''INSERT INTO ARTICLES (TITLE, CONTENT, BRIEF, DATE, ATTACHMENTS, AUTHOR, CATEGORY) VALUES (?, ?, ?, ?, ?, ?, ?);''', (JSON["title"], JSON["content"], JSON["brief"], int(time()), dumps(newFileNames), Request.userName, JSON["category"]))
	conn.commit()
	conn.close()
