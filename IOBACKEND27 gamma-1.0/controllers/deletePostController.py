...
def handleRequest(Request):
	...
	cursor = conn.cursor()
	cursor.execute("SELECT ATTACHMENTS, AUTHOR FROM POSTS WHERE ID = ?", (id,))
	data = cursor.fetchall()
	if len(data) != 1:
		conn.close()
		return Render.responseJSON('{"status": "Post not exist"}')
	...
	conn.commit()
	conn.close()
	FilesManagment.deleteFiles(loads(data[0][0]), path.join(Request.path, "views", "postsAttachments"))
